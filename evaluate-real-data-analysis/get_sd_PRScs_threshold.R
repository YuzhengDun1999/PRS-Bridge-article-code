####### This script is used for evaluation of PRS-CS-threshold ###########
    ...
    for (phi_i in 1:length(phi_list)){ 
      for (threshold in threshold_list) {
        ...
        for (seed in 1:100) {
          ...
          result[result_i,] = c(seed, phi, threshold, evaluation(tuning), evaluation(validation)); result_i = result_i + 1
        }
      }
    }
