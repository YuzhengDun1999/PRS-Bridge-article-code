...
for (ref in Ref) {
  ...
  for (phi_i in 1:length(phi_list)){  
    ...
    for (seed in 1:10) {
      ...
      result[result_i,] = c(seed, phi, evaluation(tuning), evaluation(validation)); result_i = result_i + 1
    }
  }
  ...
  readr::write_tsv(data.frame(phi_best = "auto", R2 = mean(result_auto$validation_R2), std = sqrt(var(result_auto$validation_R2))), 
                   paste0(trait, "/result/", method, '_auto_', ref, '_validation_result_std.txt'))
