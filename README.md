...
2.  Run the PRS methods on the preprocessed data:
     ...
     -   PRS-CS: Run `Rscript run-real-data-analysis/PRScs.R TRAIT`, where  `TRAIT` should match the input parameter specified in the previous step. This will generate a brunch of script files to run PRS-CS in the directory `{TRAIT}/1kg/PRScs/run_sh` and `{TRAIT}/ukbb/PRScs/run_sh`. The jobs will then be automatically submitted via SLURM. The coefficients will be stored in the directory `{TRAIT}/1kg/PRScs/result` and `{TRAIT}/ukbb/PRScs/result`.
     ...
     -   PRS-CS-Proj: Run `Rscript run-real-data-analysis/PRScs_proj.R TRAIT`, where  `TRAIT` should match the input parameter specified in the previous step. This will generate a brunch of script files to run PRS-CS-proj in the directory `{TRAIT}/1kg/PRScs_proj/run_sh` and `{TRAIT}/ukbb/PRScs_proj/run_sh`. The jobs will then be automatically submitted via SLURM. The coefficients will be stored in the directory `{TRAIT}/1kg/PRScs_proj/result` and `{TRAIT}/ukbb/PRScs_proj/result`.
     -   PRS-CS-Regularized: Run `Rscript run-real-data-analysis/PRScs_regularized.R TRAIT`, where  `TRAIT` should match the input parameter specified in the previous step. This will generate a brunch of script files to run PRS-CS-Regularized in the directory `{TRAIT}/1kg/PRScs_regularized/run_sh` and `{TRAIT}/ukbb/PRScs_regularized/run_sh`. The jobs will then be automatically submitted via SLURM. The coefficients will be stored in the directory `{TRAIT}/1kg/PRScs_regularized/result` and `{TRAIT}/ukbb/PRScs_regularized/result`.
