...
def mcmc(a, b, phi, sst_dict, n, ld_blk, blk_size, n_iter, n_burnin, thin, chrom, out_dir, beta_std, write_psi, write_pst, seed, eigenval_rm):
    # print('... MCMC ...')
    ...
    for kk in range(n_blk):
        if blk_size[kk] == 0:
            ...
            start_idx = max(sum(eigenval < 0.1), int(np.floor(eigenval.shape[0] * eigenval_rm)))
            eigenval_new = np.copy(eigenval[start_idx:]);
            eigenvec_new = np.copy(eigenvec[:, start_idx:])
    ...
    for itr in range(1, n_iter + 1):
        ...
        for kk in range(n_blk):
            if blk_size[kk] == 0:
                ...
                beta_tmp = linalg.solve_triangular(dinvt_chol, beta_mrg[idx_blk], trans='T') + sp.sqrt(
