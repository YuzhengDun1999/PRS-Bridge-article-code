...
def parse_param():
    long_opts_list = ['ref_dir=', 'bim_prefix=', 'sst_file=', 'a=', 'b=', 'phi=', 'n_gwas=',
                      'n_iter=', 'n_burnin=', 'thin=', 'out_dir=', 'chrom=', 'beta_std=', 'write_psi=', 'write_pst=', 'seed=',
                      'eigenval_rm=', 'help']
    ...
    if len(sys.argv) > 1:
        ...
        for opt, arg in opts:
            ...
            elif opt == "--eigenval_rm": param_dict['eigenval_rm'] = float(arg)
    else:
        print(__doc__)
        sys.exit(0)
...
def main():
    ...
    for chrom in param_dict['chrom']:
        ...
        mcmc_gtb_proj.mcmc(param_dict['a'], param_dict['b'], param_dict['phi'], sst_dict, param_dict['n_gwas'], ld_blk,
                           blk_size, param_dict['n_iter'], param_dict['n_burnin'], param_dict['thin'], int(chrom),
                           param_dict['out_dir'], param_dict['beta_std'], param_dict['write_psi'], param_dict['write_pst'], param_dict['seed'], param_dict['eigenval_rm'])
