...
GA = as.numeric(temp[2]) ###### taking values in 1, 2, 3
...
########### For running result for each method, you just need to change to trait to be paste0("rho", rho, "GA", GA) in run-methods section
########### Notice that we only need run 1kg with small reference LD size. You just need to run 1kg for PRSBridge, PRS-CS, and LDpred2.
