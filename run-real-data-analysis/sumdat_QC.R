...
######### Input summary statistics must have same format as in example/sumdat.txt
temp <- commandArgs(TRUE)
trait = temp[1] # phenotype name, same for the whole pipeline

sumdat = bigreadr::fread2(paste0(trait, '/sumdat.txt')) ###### Change the path to your summary statistics, it must have the same format and name as the example shown in the example/sumdat.txt
