...
#### To generate this result, following the same guidance in plot.R
...
my_plot_new = function(trait){
  result = read.table(paste0('RR.txt'), header = TRUE)
  result = result %>% filter(trait == trait)
...
ggsave("continuous_figure_RE.pdf", plot, width = 8, height = 6)
...
ggsave("disease_figure_RE.pdf", plot, width = 8, height = 6)
