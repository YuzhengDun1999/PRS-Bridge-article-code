...
###### To generate this plot, just run PRSBridge.R in run-methods folder with default option.

####### Input trait is the same as trait used in run-methods and evaluation
plot_tuning = function(trait){
  result = read.table(paste0(trait, "/Bridge_small_ukbb_all_result.txt"), header = TRUE)
...
ggsave("tuning_figure.pdf", plot, width = 8, height = 6)
...
ggsave("tuning_disease_figure.pdf", plot, width = 8, height = 6)